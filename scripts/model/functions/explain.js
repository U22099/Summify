...
// Asynchronous function to generate a text explanation using Google Gemini.
export async function TextExplanation(text, length) {
  ...
  // Gets the Gemini 1.5 Flash model with instructions for generating detailed explanations.
  ...
  // Defines the prompt for the explanation task.
  ...
  try {
    ...
    history[index].inputData = {
      ...
      length // Stores the desired length of the explanation.
    };
    history[index].outputData = response; // Stores the generated explanation.
...
// Asynchronous function to generate a file explanation using Google Gemini.
export async function FileExplanation(file, length, isNew) {
  // Creates a new Google Generative AI instance.
  const genAI = new GoogleGenerativeAI(import.meta.env.VITE_API_KEY);
  // Gets the Gemini 1.5 Flash model with explanation instructions.
  const model = genAI.getGenerativeModel({
    model: "gemini-1.5-flash",
